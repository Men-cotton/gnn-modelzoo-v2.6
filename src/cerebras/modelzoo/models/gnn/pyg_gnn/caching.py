...
class GraphCache:
    ...
    def auto_cache(self, data, percent=None):
        ...
        if percent is None:
            ...
            if self.device.type == "cuda":
                ...
                required_mem_full = self.num_nodes * self.feature_dim * 4 # float32 bytes
                ...
                print(f"[GraphCache] Auto-detected memory. Free: {free_mem/1e9:.2f}GB. "
        ...
        print(f"[GraphCache] Caching {num_cache} / {self.num_nodes} nodes ({percent*100:.1f}%) on {self.device}...")
        ...
        print(f"[GraphCache] Layout complete. Cached features size: {self.cached_x.size()}")
    ...
    def fetch(self, n_id):
        ...
        # Prepare output
        out = torch.empty((n_id.size(0), self.feature_dim), device=self.device, dtype=self.cached_x.dtype)
